...
| Package | Purpose |
|---|---|
| [`ref`](ref/) | The `op://<vault>/<item>/<field>` secret-reference type (`Ref`, `Parse`). A `Ref` is a *reference* to a secret — `Resolve` / `List` / `Lease` take refs, never raw material. `BindAuth` is the one inbound material path (see [`docs/SECURITY.md`](docs/SECURITY.md)). |
| [`cerr`](cerr/) | The connector error taxonomy: a **closed** `Kind` vocabulary (NotFound / Unauthorized / Unavailable / RateLimited / Unsupported / Invalid / Timeout / ContractViolation / Unknown), `Error`, `New`, `RateLimited`, `KindOf`, `Classified`, `Retryable`, `TripsBreaker`, and an optional non-secret `Observation` (opaque `token`/`account`/`tenant`/`egress`/`unknown` bucket, retry provenance, observed vs estimated vs unknown upstream counts). Callers classify errors by kind, never by string-matching — and `Unknown` deliberately does **not** trip a breaker. |
...
| [`credential`](credential/) | The `CredentialLoader` connector class: `Resolve` / `List` / `Lease` / `Renew` / `Revoke`, plus `Resolution` (a resolve result in which a credential that **did not resolve cannot be read as an empty one** — an empty value is expressible, but only by asserting it with `ResolvedEmpty()`), `Material` (redacted, revealable-on-demand, wipeable secret bytes), `Lease`, the optional `BatchResolver` operation behind `CapBatchResolve`, the optional `AuthBinder` operation behind `CapBindAuth`, and the optional `BundleAcquirer` operation behind `CapGrantBundle`, and the optional `AuthLifecycle` operation behind `CapAuthLifecycle` (provider-auth expiry, not a dynamic-secret `Lease`). |
...
| [`manifest`](manifest/) | The `connector.yaml` schema: `name`, `implements`, `kind`, typed `capabilities`, refs-only `auth`, `min_host_version`. Strict parse, closed enums. |
...
| [`skillspec`](skillspec/) | The published `skill.yml` contract (`Skill`, `Parse`, `Validate`): catalogue metadata including `version` and `capability_class`, closed vocabularies, strict unknown-field rejection. Bytes in, Skill out — no filesystem discovery, version selection, or client-native emission. Standalone — not imported by the connector packages. |
| [`kernel`](kernel/) | **Not a connector contract.** The deterministic act-kernel semantics, shared by the runtime and the verifier. ⚠️ **Per-package status, because they differ** — [`canonical`](kernel/canonical/) **implemented** (the encoding whose bytes an act's identity is a hash of: JSON numbers forbidden, domain-separation tags, committed vectors, and the DSSE envelope with `act_body_id` over the *unsigned* body); [`tapeformat`](kernel/tapeformat/) **implemented** (the act chain's entry shape, chain rule and **reader** — truncation, gaps, broken links and unknown fields all refused; the write path stays in the runtime); [`reduce`](kernel/reduce/) **implemented** (the one pure reducer — no filesystem, environment, clock or network, returning rather than invoking at any depth. It runs every rule it declares and accepts what none refused: the prefix chains, its acceptance times neither regress nor cross clock authorities, the candidate is not a replay, its permit traces to an act that was actually accepted and is unspent, and an independent **observation** reports the tree the act binds — a host's own *receipt* does not satisfy that gate. ⚠️ It checks a permit's **lineage**, not its **scope**: a candidate carries a permit *outpoint* and not a permit body, so this package has nothing to hand `Grant.Attenuates` — attenuation is checked where permits are *issued*, and the residual is that lineage only holds if every permit reached the tape through issuance; nor the canonical head, which is a publisher's compare-and-swap and not a reducer's); [`predicate`](kernel/predicate/) and [`keyhistory`](kernel/keyhistory/) **skeletons**, declared and documented, not implemented. |
| [`contracts`](contracts/) | **Not a connector contract.** The public record types an outsider needs to read a bundle. Four layers kept apart by their mutation rules — `ActSpec` immutable, `Witness` and `EvidenceEvent` append-only, `StatusView` disposable and authoritative for nothing. ⚠️ `Decode` **rejects unknown fields**: a field the verifier skips may be one the runtime acted on, so accepting it means agreeing on the bytes while disagreeing on the meaning. JSON Schemas in `contracts/schema/` set `additionalProperties:false` so a non-Go consumer reaches the same verdict, and the two are checked against each other in both directions; golden fixtures in `contracts/testdata/` pin the wire form. |
...
| [`repoobs`](repoobs/) | **Not a connector contract.** The versioned repository observation (`SchemaVersion` 1): ResourceID, source identity and storage trust domain, optional Session/TreeSet, four git axes, and index freshness as a separate projection. Unknown, denied, unsupported and measured zero are distinct; a WorkEvent envelope round-trips the legacy fixture. Core may consume this only after its module pin includes the tagged implementation. |
...
`connector.yaml`, and an in-process conformance test — **compiles and passes
...
hardcoded honest rather than derived from `connector.yaml`. It also does not,
...
| `bind_auth/declared-is-implemented` | `bind_auth` is declared exactly when `credential.AuthBinder` is implemented |
| `grant_bundle/declared-is-implemented` | `grant_bundle` is declared exactly when `credential.BundleAcquirer` is implemented |
| `grant_bundle/complete-inventory` | a declared grant bundle is ready only with every enrolled key present |
| `auth_lifecycle/declared-is-implemented` | `auth_lifecycle` is declared exactly when `credential.AuthLifecycle` is implemented |
| `auth_lifecycle/not-secret-lease` | static auth does not advertise `CapLease`; wrong handle kinds fail |
