...
| [`kernel`](kernel/) | **Not a connector contract.** The deterministic act-kernel semantics, shared by the runtime and the verifier. ⚠️ **Per-package status, because they differ** — [`canonical`](kernel/canonical/) **implemented** (the encoding whose bytes an act's identity is a hash of: JSON numbers forbidden, domain-separation tags, committed vectors, and the DSSE envelope with `act_body_id` over the *unsigned* body); [`tapeformat`](kernel/tapeformat/) **implemented** (the act chain's entry shape, chain rule and **reader** — truncation, gaps, broken links and unknown fields all refused; the write path stays in the runtime); [`reduce`](kernel/reduce/) **implemented** (the one pure reducer — no filesystem, environment, clock or network, returning rather than invoking at any depth. It runs every rule it declares and accepts what none refused: the prefix chains, its acceptance times neither regress nor cross clock authorities, the candidate is not a replay, its permit is unspent, and an independent **observation** reports the tree the act binds — a host's own *receipt* does not satisfy that gate. ⚠️ It does **not** check non-amplification, because a candidate carries a permit *outpoint* and not a permit body, so this package has no scope to compare; nor the canonical head, which is a publisher's compare-and-swap and not a reducer's); [`predicate`](kernel/predicate/) and [`keyhistory`](kernel/keyhistory/) **skeletons**, declared and documented, not implemented. |
