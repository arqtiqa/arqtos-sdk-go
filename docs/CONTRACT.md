...
`Class.Valid()` rejects anything outside it, and the `connector.yaml`
...
| `exact_ref` | `ExactRefTransport` | `ReadExact(ctx, ExactReadRequest) (ExactReadResult, error)` and `CompareAndSwap(ctx, CASRequest) (CASReceipt, error)` — the exact revision of one explicit ref, and an expected-head update of that same ref, addressed by `Realm` + `NativeID` (never a path). `CASRequest.SourceDir` is the explicit absolute local Git repository that already contains `New` and its reachable object closure; it is read-only, never the index or uncommitted files, and never an ambient current directory. The connector stages objects in isolated storage without source hooks, inherited credentials, arbitrary local Git configuration, or alternates. Path syntax validation does not prove those properties. A source-less CAS is `cerr.KindInvalid`. ⚠️ All-or-nothing: write support without the matching exact read cannot recover `CASIndeterminate`. A mismatch is a `CASMismatch` receipt with `Observed`, not an error. Cancellation is `cerr.KindTimeout` and MUST NOT be reported as a write result. An indeterminate receipt is not success and MUST NOT be retried as applied; the caller recovers with `ReadExact`. Existing connectors remain compatible until they declare this capability. |
