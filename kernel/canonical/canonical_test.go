...
func TestDomain_ValidIsClosed(t *testing.T) {
	...
	for _, d := range []canonical.Domain{"", "arqtos.act-body", "act-body.v1", "arqtos.act-body.v2", "arqtos.resolved-config", "arqtos.resolved-config.v2"} {
		if d.Valid() {
...
func TestDomainResolvedConfig_IsTheSixthMemberAndSeparatesFromTheOthers(t *testing.T) {
	...
	if members != 1 || len(canonical.Domains()) != 6 {
		t.Fatalf("Domains() lists the resolved-config domain %d time(s) among %d; want once among six", members, len(canonical.Domains()))
	}
